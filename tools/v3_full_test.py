...
try:
    ...
    from src.backend.config import load_settings, save_settings as persist_settings, LANGUAGES, CONFIG_DIR
...
test("settings has all required keys", lambda: all(
    k in api.get_settings() for k in
    ["mode", "model", "language", "theme", "auto_copy", "start_with_windows",
     "hotkey", "close_behavior", "sound_enabled", "always_on_top", "show_pill_widget",
     "output_mode", "target_language", "source_language", "pill_monitor", "auto_copy_target"]
))
...
test("save_settings preserves existing keys", lambda: (
...
test("get_models has expected entries", lambda: (
    {m["name"] for m in api.get_models()} >= {"tiny", "base", "small", "medium", "large", "turbo"}
))

test("each model has required fields", lambda: all(
    all(k in m for k in ["name", "size", "downloaded"]) for m in api.get_models()
))
...
test("each mic has id and name", lambda: all(
    "id" in m and "name" in m for m in api.get_microphones()
))
...
if mics:
    ...
    test("set_microphone to invalid id (-999)", lambda: (
...
test("get_languages has auto-detect", lambda: any(
    lang.get("code") == "auto" for lang in api.get_languages()
))

test("get_languages has English", lambda: any(
    lang.get("code") == "en" for lang in api.get_languages()
))
...
test("append_history_item", lambda: api.append_history_item(
    "Test transcript from V3 test suite", duration=2.5, transcription_time=0.8
).get("success"))

test("history count increased by 1", lambda: len(api.get_history()) == initial_count + 1)

test("appended item has correct text", lambda: (
    api.get_history()[0]["text"] == "Test transcript from V3 test suite"
))

test("appended item has transcription_time", lambda: (
    api.get_history()[0].get("transcription_time") == 0.8
))

test("appended item has duration", lambda: (
    api.get_history()[0].get("duration") == 2.5
))
...
test("append_history_item with special chars", lambda: api.append_history_item(
...
test("copy_to_clipboard special chars", lambda: api.copy_to_clipboard(
    'Line 1\nLine 2\tTabbed\n"Quoted" & <tagged>'
).get("success"))

test("copy_to_clipboard unicode", lambda: api.copy_to_clipboard(
    "Hello 你好 مرحبا 🎤"
).get("success"))
...
time.sleep(1.0)  # Wait for cancel tone to finish (sounddevice race condition)

test("get_audio_level after cancel is 0", lambda: api.get_audio_level() == 0.0)

time.sleep(1.5)  # Wait for cancel tone to fully finish (sounddevice heap safety)
...
test("verify_api_key with dummy key returns dict", lambda: isinstance(
    api.verify_api_key("openai", "sk-dummy-test-key-not-real"), dict
))

test("verify_api_key dummy key is not valid", lambda: not api.verify_api_key(
    "openai", "sk-dummy-test-key-not-real"
).get("valid", True))

test("verify_api_key empty key", lambda: isinstance(
    api.verify_api_key("openai", ""), dict
))
...
test("cancel request flow", lambda: (
...
test("set_api_credentials openai", lambda: (
...
test("set_api_credentials gemini", lambda: (
...
def test_save_settings_syncs_provider():
    ...
    ok = (api._settings.get("api_provider") == "gemini"
          and api._settings.get("api_model") == "gemini-2.5-flash")
...
test("cancel_processing plays cancel tone", lambda: (
    api.cancel_processing().get("success") is True
))
...
def test_save_settings_syncs_output():
    ...
    ok = (api._settings.get("output_mode") == "translate"
          and api._settings.get("target_language") == "es")
    # RESTORE original values
    api.save_settings({
...
test("get_audio_numpy length matches duration", lambda: (
    abs(len(rec.get_audio_numpy()) / 16000 - rec.get_duration()) < 0.1
))
...
test("persist_settings and reload", lambda: (
...
test("MODEL_INFO has expected models", lambda: all(
    m in model_manager.MODEL_INFO for m in ["tiny", "base", "small", "medium", "large", "turbo"]
))

test("each model info has size", lambda: all(
    "size" in info for info in model_manager.MODEL_INFO.values()
))

for name in ["tiny", "base"]:
    test(f"is_model_downloaded({name}) returns bool", lambda n=name: isinstance(
        model_manager.is_model_downloaded(n), bool
    ))

test("get_downloaded_models returns list", lambda: isinstance(
    model_manager.get_downloaded_models(), list
))
...
test("save_settings with None value", lambda: isinstance(
    api.save_settings({"theme": None}), dict
))
# Restore
api.save_settings({"theme": "dark"})

test("save_settings with extra unknown key", lambda: api.save_settings(
...
test("rapid start/cancel x3", lambda: all([
    (api.start_recording(), time.sleep(0.1), api.cancel_recording())
    and True for _ in range(3)
]))
...
for i, (l, t, r, b) in enumerate(monitors):
    print(f"    Monitor {i}: ({l},{t})->({r},{b}) [{r-l}x{b-t}]")
...
for i, (l, t, r, b) in enumerate(monitors):
    cx, cy = (l + r) // 2, (t + b) // 2
    test(f"monitor {i} center ({cx},{cy}) on screen",
         lambda cx=cx, cy=cy: _is_pos_on_screen(cx, cy, monitors))

# Test each monitor's near-boundary positions
for i, (l, t, r, b) in enumerate(monitors):
    # Just inside top-left
    test(f"monitor {i} top-left ({l},{t}) on screen",
         lambda l=l, t=t: _is_pos_on_screen(l, t, monitors))
    # 51px from right edge (should be on screen)
    test(f"monitor {i} near-right ({r-51},{t}) on screen",
         lambda r=r, t=t: _is_pos_on_screen(r - 51, t, monitors))
    # Within 50px of right edge (should be off — would be barely visible)
    test(f"monitor {i} at-right-margin ({r-50},{t}) off screen",
         lambda r=r, t=t: not _is_pos_on_screen(r - 50, t, monitors))
...
test("get_monitors entries have required fields", lambda: all(
    all(k in m for k in ["index", "name", "resolution", "primary", "label"])
    for m in api.get_monitors()
))
test("get_monitors has one primary", lambda: sum(
    1 for m in api.get_monitors() if m["primary"]
) == 1)
test("get_monitors indices sequential", lambda: (
    [m["index"] for m in api.get_monitors()] == list(range(len(api.get_monitors())))
))
...
def test_get_audio_returns_base64():
    ...
    from src.backend.config import load_history, save_history as persist_history
...
test("get_recording_state idle is_recording is False",
     lambda: api.get_recording_state()["is_recording"] is False)

test("get_recording_state idle cancel_requested is False",
     lambda: api.get_recording_state()["cancel_requested"] is False)
...
test("parse Ctrl + Alt + R",
     lambda: _parse_hotkey_to_win32("Ctrl + Alt + R") is not None)
test("parse Ctrl + Shift + R",
     lambda: _parse_hotkey_to_win32("Ctrl + Shift + R") is not None)
test("parse Ctrl + F9",
     lambda: _parse_hotkey_to_win32("Ctrl + F9") is not None)
test("parse F8",
     lambda: _parse_hotkey_to_win32("F8") is not None)
test("parse Shift + F9",
     lambda: _parse_hotkey_to_win32("Shift + F9") is not None)

# Blocked combos must return None
test("blocked Ctrl + C returns None",
     lambda: _parse_hotkey_to_win32("Ctrl + C") is None)
test("blocked Alt + F4 returns None",
     lambda: _parse_hotkey_to_win32("Alt + F4") is None)
test("blocked Ctrl + V returns None",
     lambda: _parse_hotkey_to_win32("Ctrl + V") is None)
test("blocked Ctrl + Shift + Esc returns None",
     lambda: _parse_hotkey_to_win32("Ctrl + Shift + Esc") is None)

# Edge cases
test("empty string returns None",
     lambda: _parse_hotkey_to_win32("") is None)
test("None returns None",
     lambda: _parse_hotkey_to_win32(None) is None)
test("modifier-only returns None",
     lambda: _parse_hotkey_to_win32("Ctrl") is None)
...
def test_pill_queue_when_unavailable():
    ...
    try:
        ...
        assert api._pending_pill_state == "dormant", \
            f"Expected 'dormant', got {api._pending_pill_state}"
    finally:
...
def test_pill_queue_overwrites():
    ...
    try:
        ...
        api._notify_pill("dormant")
        assert api._pending_pill_state == "dormant", \
            f"Should overwrite to 'dormant', got {api._pending_pill_state}"
    finally:
        api._pill_manager = saved_pm
        api._pending_pill_state = None
    return True
...
def test_pill_queue_replay():
    ...
    try:
        ...
        assert api._pending_pill_state == "recording", \
            "Queue should persist when pill_manager set to None"
...
def test_state_start_cancel_sequence():
    ...
    assert s2["cancel_requested"] is False or s2["cancel_requested"] is True, \
        "cancel_requested should be bool"
    return True
...
def test_hotkey_bind_stop_cycle():
    ...
    ok = listener.bind("Ctrl + Alt + R")
    assert ok, "bind should succeed"
    listener.stop()
...
def test_hotkey_rebind():
    ...
    listener = Win32HotkeyListener(lambda: None)
    ok1 = listener.bind("Ctrl + Alt + R")
    assert ok1
    ok2 = listener.bind("Ctrl + Alt + D")
...
def test_hotkey_rapid_rebind():
    ...
    listener = Win32HotkeyListener(lambda: None)
    combos = ["Ctrl + Alt + R", "Ctrl + Alt + D", "Ctrl + F9", "F8", "Shift + F9"]
    for combo in combos:
...
def test_hotkey_conflict_detection():
    ...
    ok1 = l1.bind("Ctrl + Alt + D")
    assert ok1, "first bind should succeed"
    ok2 = l2.bind("Ctrl + Alt + D")
...
# Duplicate modifiers
test("duplicate modifier Ctrl+Ctrl+R still parses",
     lambda: _parse_hotkey_to_win32("Ctrl + Ctrl + R") is not None)
...
# Mixed case
test("lowercase ctrl+alt+r parses",
     lambda: _parse_hotkey_to_win32("ctrl+alt+r") is not None)
test("UPPERCASE CTRL+ALT+R parses",
     lambda: _parse_hotkey_to_win32("CTRL+ALT+R") is not None)
test("mixed case Ctrl+aLt+r parses",
     lambda: _parse_hotkey_to_win32("Ctrl+aLt+r") is not None)
...
# Extra whitespace
test("extra spaces Ctrl  +  Alt  +  R parses",
     lambda: _parse_hotkey_to_win32("Ctrl  +  Alt  +  R") is not None)
test("leading/trailing spaces parse",
     lambda: _parse_hotkey_to_win32("  Ctrl + Alt + R  ") is not None)
test("tabs in hotkey string parse",
     lambda: _parse_hotkey_to_win32("Ctrl\t+\tAlt\t+\tR") is not None)

# Numeric keys
test("parse Ctrl + 0",
     lambda: _parse_hotkey_to_win32("Ctrl + 0") is not None)
test("parse Ctrl + 9",
     lambda: _parse_hotkey_to_win32("Ctrl + 9") is not None)
test("parse Ctrl + Alt + 5",
     lambda: _parse_hotkey_to_win32("Ctrl + Alt + 5") is not None)

def test_numeric_key_vk():
    mods, vk = _parse_hotkey_to_win32("Ctrl + 0")
    assert vk == 0x30, f"VK for 0 should be 0x30, got {hex(vk)}"
    mods, vk = _parse_hotkey_to_win32("Ctrl + 9")
...
test("invalid modifier Super+R returns None",
     lambda: _parse_hotkey_to_win32("Super + R") is None)
test("invalid modifier Hyper+R returns None",
     lambda: _parse_hotkey_to_win32("Hyper + R") is None)
test("gibberish modifier Foo+R returns None",
     lambda: _parse_hotkey_to_win32("Foo + R") is None)

# Special keys
test("parse Ctrl + Space",
     lambda: _parse_hotkey_to_win32("Ctrl + Space") is not None)
test("parse Ctrl + Tab returns None (blocked)",
     lambda: _parse_hotkey_to_win32("Ctrl + Tab") is None)
test("parse Ctrl + Alt + Delete returns None (blocked)",
     lambda: _parse_hotkey_to_win32("Ctrl + Alt + Delete") is None)

# F-key range (F1-F6 and F11 are blocked)
test("parse F1 returns None (blocked)",
     lambda: _parse_hotkey_to_win32("F1") is None)
test("parse F7 is allowed",
     lambda: _parse_hotkey_to_win32("F7") is not None)
test("parse F24 is allowed",
     lambda: _parse_hotkey_to_win32("F24") is not None)

def test_fkey_vk_range():
    mods7, vk7 = _parse_hotkey_to_win32("F7")
    mods24, vk24 = _parse_hotkey_to_win32("F24")
...
test("just + returns None",
     lambda: _parse_hotkey_to_win32("+") is None)
test("empty between plus Ctrl++R returns None",
...
# Basic returns
test("guess empty text returns auto",
     lambda: _ApiCls._guess_language("", "en") == "auto")
test("guess None returns auto",
     lambda: _ApiCls._guess_language(None, "en") == "auto")
test("guess short text (<5 chars) returns auto",
     lambda: _ApiCls._guess_language("hi", "en") == "auto")
test("guess whitespace-only returns auto",
     lambda: _ApiCls._guess_language("     ", "en") == "auto")

# CJK (Chinese)
test("guess Chinese text returns zh",
     lambda: _ApiCls._guess_language("today is sunny", "en") != "zh")
test("guess pure Chinese chars returns zh",
     lambda: _ApiCls._guess_language("\u4f60\u597d\u4e16\u754c\u4eba\u6c11", "en") == "zh")

# Japanese (CJK + Kana)
test("guess Japanese with kana returns ja",
     lambda: _ApiCls._guess_language("\u3053\u3093\u306b\u3061\u306f\u4e16\u754c", "en") == "ja")
test("guess pure hiragana returns ja",
     lambda: _ApiCls._guess_language("\u3042\u3044\u3046\u3048\u304a\u304b\u304d", "en") == "ja")
test("guess pure katakana returns ja",
     lambda: _ApiCls._guess_language("\u30a2\u30a4\u30a6\u30a8\u30aa\u30ab\u30ad", "en") == "ja")

# Korean
test("guess Korean text returns ko",
     lambda: _ApiCls._guess_language("\uc548\ub155\ud558\uc138\uc694 \uc138\uacc4", "en") == "ko")

# Arabic
test("guess Arabic text returns ar",
     lambda: _ApiCls._guess_language("\u0645\u0631\u062d\u0628\u0627 \u0627\u0644\u0639\u0627\u0644\u0645", "en") == "ar")

# Devanagari (Hindi)
test("guess Hindi text returns hi",
     lambda: _ApiCls._guess_language("\u0928\u092e\u0938\u094d\u0924\u0947 \u0926\u0941\u0928\u093f\u092f\u0627", "en") == "hi")

# Latin-script dominant
test("guess English text with target_lang=en returns en",
     lambda: _ApiCls._guess_language("This is a simple English sentence for testing", "en") == "en")
test("guess English text with target_lang=fr returns auto",
     lambda: _ApiCls._guess_language("This is a simple English sentence for testing", "fr") == "auto")

# Punctuation/numbers only
test("guess numbers-only returns auto",
     lambda: _ApiCls._guess_language("12345 67890 12345", "en") == "auto")
test("guess punctuation-only returns auto",
     lambda: _ApiCls._guess_language("!@#$% ^&*() !@#$%", "en") == "auto")

# Mixed scripts
test("guess mixed English+CJK (mostly English) returns auto or en",
     lambda: _ApiCls._guess_language("Hello world this is mostly English with one \u4f60", "en") in ("en", "auto"))
test("guess mixed English+CJK (mostly CJK) returns zh",
...
test("normalize_provider openai returns openai",
     lambda: _ApiCls._normalize_provider("openai") == "openai")
test("normalize_provider OPENAI returns openai",
     lambda: _ApiCls._normalize_provider("OPENAI") == "openai")
test("normalize_provider gemini returns gemini",
     lambda: _ApiCls._normalize_provider("gemini") == "gemini")
test("normalize_provider with spaces returns openai",
     lambda: _ApiCls._normalize_provider("  openai  ") == "openai")
test("normalize_provider None returns None",
     lambda: _ApiCls._normalize_provider(None) is None)
test("normalize_provider empty string returns None",
     lambda: _ApiCls._normalize_provider("") is None)
test("normalize_provider invalid returns None",
     lambda: _ApiCls._normalize_provider("azure") is None)
test("normalize_provider number returns None",
     lambda: _ApiCls._normalize_provider("123") is None)
...
test("_keyring_available returns bool",
     lambda: isinstance(_ApiCls._keyring_available(), bool))
...
def test_cleanup_preserves_valid_files():
    ...
    from src.backend.config import AUDIO_DIR, load_history, save_history as persist_history_fn
...
def test_cleanup_strips_stale_references():
    ...
    from src.backend.config import load_history, save_history as persist_history_fn
...
def test_tray_icons_differ():
    ...
    assert list(get_data()) != list((getattr(img_record, 'get_flattened_data', None) or img_record.getdata)()), \
...
from src.backend.config import _migrate_config, CONFIG_DIR
...
def test_settings_not_corrupted():
    ...
    issues = []
    for key in ["mode", "api_provider", "api_model", "output_mode",
                 "target_language", "source_language", "theme", "api_base_url"]:
