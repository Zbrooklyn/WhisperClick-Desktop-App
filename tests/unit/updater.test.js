...
describe('event forwarding', () => {
  ...
  test('a missing update feed reports unavailable, not a red error', () => {
    // A build handed out before its feed is published 404s on every launch.
    // That must not surface as "Update check failed" in the user's settings.
    for (const msg of [
      'Cannot find channel "beta.yml" update info: HttpError: 404 Not Found',
      'getaddrinfo ENOTFOUND whisperclick.com',
      'connect ECONNREFUSED 127.0.0.1:443',
      "ENOENT: no such file or directory, open 'C:\\app\\resources\\app-update.yml'",
    ]) {
      mainWindow.webContents.send.mockClear();
      autoUpdater.emit('error', new Error(msg));
      expect(mainWindow.webContents.send).toHaveBeenCalledWith(
        'update-status', expect.objectContaining({ status: 'unavailable' })
      );
    }
  });
