...
describe('close-to-tray behavior', () => {
  test('close with closeBehavior=tray prevents default and hides window', async () => {
    ...
    // Bring the window back before leaving this test. Closing to tray sets
    // main.js's module-level `pillPromoted` flag, and real Electron clears it by
    // emitting 'show' when the window returns — a step this mock only performs
    // if we ask it to. Skipping it left the flag stuck on for the remainder of
    // the file, so pillShouldShow() answered true forever and the later
    // showPill=false test never saw the pill close. The product was right; the
    // test had simply hidden the window and never brought it back.
    if (mainWin._listeners['show']) mainWin._listeners['show'].forEach(fn => fn());
  });
