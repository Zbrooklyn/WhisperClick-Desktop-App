...
class TranscriptionService:
    def __init__(self):
        ...
        self._api_base_url = ""
        # WC-PREMIUM:START
        # Managed ("Complete" tier): proxy URL + license token instead of a user
        # API key. Inert until both are set via set_managed_credentials.
        self._managed_proxy_url = ""
        self._license_token = ""
        # WC-PREMIUM:END
    ...
    # WC-PREMIUM:START
    def set_managed_credentials(self, proxy_url, license_token):
        """Configure the managed ("Complete" tier) provider.

        The proxy (e.g. a Cloudflare Worker) holds the real provider key
        server-side; the user authenticates to it with their subscription
        license token (Bearer). No personal OpenAI/Gemini key is needed. Pass
        empty strings to clear. Selecting the provider itself is done via
        set_api_credentials(provider="managed", ...).
        """
        self._managed_proxy_url = (proxy_url or "").rstrip("/")
        self._license_token = license_token or ""
    # WC-PREMIUM:END
    ...
    def _transcribe_api(self, wav_bytes):
        ...
        # WC-PREMIUM:START
        if provider == "managed":
            # No user API key: the proxy authenticates with the license token.
            return self._transcribe_managed(wav_bytes)
        # WC-PREMIUM:END
    ...
    # WC-PREMIUM:START
    # ------------------------------------------------------------------
    # Managed ("Complete" tier) provider
    #
    # Audio transcription and LLM calls both go through a proxy (e.g. a
    # Cloudflare Worker) that holds the real provider key server-side. The user
    # authenticates with their subscription license token (Bearer) — no personal
    # API key. The request contract the proxy must implement:
    #
    #   POST {proxy}/v1/transcribe    Authorization: Bearer <license_token>
    #     body : {"audio": <base64>, "mime_type": str, "language"?: str,
    #             "prompt"?: str, "word_timestamps": bool}
    #     200  : {"text": str, "segments"?: [...], "words"?: [...], "language"?: str}
    #
    #   POST {proxy}/v1/chat          Authorization: Bearer <license_token>
    #     body : {"messages": [{"role","content"}...], "temperature": float}
    #     200  : {"text": str}   (an OpenAI-style {"choices":[...]} is also accepted)
    #
    #   Error codes surfaced to the user: 401/403 = license invalid/expired,
    #   402 = out of plan quota, 429 = rate limited.
    #
    # TODO(edward): confirm the deployed Worker's route paths match these two
    # constants once the proxy is live; the proxy base URL comes from config
    # (managed_proxy_url) so nothing here is hard-coded to a deployment.
    # ------------------------------------------------------------------

    _MANAGED_TRANSCRIBE_PATH = "/v1/transcribe"
    _MANAGED_CHAT_PATH = "/v1/chat"

    def _managed_post(self, path, body, timeout=60):
        """POST a JSON body to the managed proxy; return the parsed JSON dict.

        Fail-safe: raises ValueError with a plain-English message the UI can
        show (never leaks the license token). Maps 401/403/402/429 to their
        meaning so the user knows whether it's a login, quota, or rate issue.
        """
        if not self._managed_proxy_url:
            raise ValueError(
                "Managed mode isn't configured yet. Sign in to your Complete "
                "subscription to enable it."
            )
        if not self._license_token:
            raise ValueError(
                "Your Complete subscription license is missing or expired. "
                "Sign in again to refresh it."
            )
        url = self._managed_proxy_url + path
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {self._license_token}",
        }
        ...
        try:
            ...
            try:
                ...
                detail = ""
            safe = redact_key(detail, self._license_token, limit=300)
            if code in (401, 403):
                raise ValueError(
                    "Your Complete subscription license is invalid or expired. "
                    "Sign in again to continue."
                ) from None
            if code == 402:
                raise ValueError(
                    "Your Complete plan is out of included usage. Upgrade your "
                    "plan or wait for your quota to reset."
                ) from None
            if code == 429:
                raise ValueError(
                    "The managed service is busy right now (rate limit). "
                    "Wait a moment and try again."
                ) from None
            raise ValueError(f"Managed service error ({code}): {safe}") from None
        except urllib_error.URLError:
            raise ValueError(
                "Can't reach the managed transcription service. Check your "
                "internet connection."
            ) from None

    def _transcribe_managed(self, wav_bytes):
        audio_file = self._compress_audio(wav_bytes)
        audio_data = audio_file.read()
        mime_type = "audio/ogg" if getattr(audio_file, "name", "").endswith(".ogg") else "audio/wav"
        body = {
            ...
            "mime_type": mime_type,
            "word_timestamps": bool(self._ext_want_word_timestamps),
        }
        if self._language:
            body["language"] = self._language
        if self._ext_initial_prompt:
            body["prompt"] = self._ext_initial_prompt

        payload = self._managed_post(self._MANAGED_TRANSCRIBE_PATH, body, timeout=120)
        if self.is_cancel_requested():
            raise TranscriptionCancelled("Transcription cancelled")
        if not isinstance(payload, dict):
            raise ValueError("Managed service returned an unexpected response format.")
        lang = payload.get("language")
        if lang:
            self.detected_language = str(lang).lower()[:2] or None
        # Preserve speaker segments / word timestamps when the proxy returns them.
        self.last_segments = self._collect_segments(payload.get("segments"))
        self.last_words = self._collect_words(payload.get("words"))
        text = payload.get("text")
        if text is None:
            raise ValueError("Managed service returned no transcript.")
        return str(text).strip()

    def _managed_chat(self, system_prompt, user_content, temperature=0):
        """One LLM call via the managed proxy (translation / summarization)."""
        body = {
            "messages": [
        ...
        payload = self._managed_post(self._MANAGED_CHAT_PATH, body, timeout=60)
        if isinstance(payload, dict):
            text = payload.get("text")
            if text is None:
                # Tolerate an OpenAI-style chat-completion passthrough.
                try:
    ...
    # ------------------------------------------------------------------
    # Provider-agnostic single LLM call (used by meeting summarization)
    # ------------------------------------------------------------------

    def chat_complete(self, system_prompt, user_content, temperature=0.2, quality="cheap"):
        """Run one LLM completion through whichever client the user configured.

        Routes to OpenAI chat, Gemini generateContent, or the managed proxy —
        the same paths translation uses. `quality` ('cheap'|'smart') trades
        speed/cost for a stronger model on OpenAI. Raises ValueError on missing
        credentials or provider errors; callers decide whether to degrade.
        """
        if not user_content or not user_content.strip():
            return ""
        provider = self._api_provider or "openai"
        if provider == "managed":
            return self._managed_chat(system_prompt, user_content, temperature)
        if provider == "gemini":
            return self._chat_gemini(system_prompt, user_content, temperature, quality)
        return self._chat_openai(system_prompt, user_content, temperature, quality)

    def _chat_openai(self, system_prompt, user_content, temperature=0.2, quality="cheap"):
        api_key = self._api_key or os.getenv("OPENAI_API_KEY", "")
        if not api_key:
            raise ValueError("OpenAI API key not set. Add it in Settings > API Keys.")
        client = self._get_openai_client(api_key, self._api_base_url)
        try:
            response = client.chat.completions.create(
                model=("gpt-4o" if quality == "smart" else "gpt-4o-mini"),
                temperature=temperature,
                messages=[
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": user_content},
    ...
    def _chat_gemini(self, system_prompt, user_content, temperature=0.2, quality="cheap"):
    ...
    def translate(self, text, target_language, source_language="auto"):
        ...
        # WC-PREMIUM:START
        if provider == "managed":
            return self._translate_managed(text, target_language, source_language)
        # WC-PREMIUM:END
        if provider == "gemini":
    ...
    # WC-PREMIUM:START
    def _translate_managed(self, text, target_language, source_language="auto"):
        ...
        out = self._managed_chat(system_prompt, text)
        if self.is_cancel_requested():
            raise TranscriptionCancelled("Translation cancelled")
        return out
    # WC-PREMIUM:END
