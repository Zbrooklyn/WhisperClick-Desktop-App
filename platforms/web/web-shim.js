...
(function () {
  ...
  var api = {
    ...
    // WC-PREMIUM:START
    // Meeting mode: summarize a transcript into summary + action items (LLM).
    summarize: function (text, style, noteType) { return jpost('/api/summarize', { text: text, style: style || 'bullets', note_type: noteType || 'auto' }); },

    // Speaker diarization: label who-said-what on a stored transcript's audio.
    diarize: function (id) { return jpost('/api/diarize', { id: id }); },

    // Chat with a note: ask a question grounded in a transcript.
    chat_note: function (transcript, question, history, quality) { return jpost('/api/chat', { transcript: transcript, question: question, history: history || '', quality: quality || 'cheap' }); },

    // Prompt/action library: transform a transcript per a one-tap instruction.
    run_action: function (transcript, action) { return jpost('/api/action', { transcript: transcript, action: action }); },
    suggest_followups: function (transcript, summary) { return jpost('/api/followups', { transcript: transcript, summary: summary || '' }); },
    extract_review_details: function (transcript) { return jpost('/api/review-details', { transcript: transcript }); },
    speaker_summaries: function (groups) { return jpost('/api/speaker-summaries', { groups: groups }); },
    key_quotes: function (transcript) { return jpost('/api/key-quotes', { transcript: transcript }); },

    // Import from a URL: fetch a direct audio/video URL and transcribe it.
    // opts?: { start, end } — seconds; transcribe only that window (0 = unset).
    import_url: function (url, opts) {
      var o = opts || {};
      return jpost('/api/import-url', { url: url, start: Number(o.start) || 0, end: Number(o.end) || 0 });
    },
    url_preview: function (url) { return jpost('/api/url-preview', { url: url }); },

    // Re-transcribe a stored recording with a better (API) model.
    // vocab?: custom-vocabulary spelling hints (names/terms) for this run.
    retranscribe: function (id, provider, vocab) { return jpost('/api/retranscribe', { id: id, provider: provider || '', vocab: vocab || '' }); },
    // WC-PREMIUM:END
    ...
    // WC-PREMIUM:START
    // Send a transcript to Slack / Notion via the server (tokens stay server-side).
    send_integration: function (text, target) { return jpost('/api/integration', { text: text, target: target }); },
    // WC-PREMIUM:END
    ...
    // WC-PREMIUM:START
    // Meeting mode / pro — inert on web for now.
    // Live meeting mode: VAD-driven long-form capture; segments accumulate into
    // a live transcript, saved as one entry on stop.
    start_meeting: function () {
      return startMeeting().then(function () { return { success: true }; })
        .catch(function (e) { return { success: false, error: e.message }; });
    },
    stop_meeting: function (extra) {
      return stopMeeting().then(function (m) {
        var text = m.text, duration = m.duration, blob = m.blob;
        if (!text || !text.trim()) return { success: true, text: '', id: null };
        function save(audioWav) {
          var payload = { text: text, duration: duration, audioWav: audioWav || null };
          if (extra && typeof extra === 'object') {
            if (Array.isArray(extra.bookmarks) && extra.bookmarks.length) payload.bookmarks = extra.bookmarks;
            if (extra.chapters) payload.chapters = extra.chapters;
            if (Array.isArray(extra.liveActionItems) && extra.liveActionItems.length) payload.liveActionItems = extra.liveActionItems;
            if (extra.speakers && Array.isArray(extra.speakers.segments) && extra.speakers.segments.length) payload.speakers = extra.speakers;
          }
          return jpost('/api/meeting-save', payload)
            .then(function (r) { return { success: true, text: text, id: r && r.id }; });
        }
        if (!blob || !blob.size) return save(null);
        // Upload the full recording first, then link it on save. Audio is optional —
        // if the upload fails, we still persist the transcript.
        return fetch(API + '/api/meeting-audio', { method: 'POST', headers: { 'Content-Type': blob.type || 'audio/webm' }, body: blob })
          .then(function (r) { return r.json(); })
          .then(function (j) { return save(j && j.success ? j.wav : null); })
          .catch(function () { return save(null); });
      });
    },
    pause_meeting: function () {
      return Promise.resolve(pauseMeeting());
    },
    resume_meeting: function () {
      return Promise.resolve(resumeMeeting());
    },
    // System / tab audio capture.
    start_system_capture: function () {
      return startSystemCapture().then(function () { fireState('recording'); return { success: true }; })
        .catch(function (e) { return { success: false, error: e.message }; });
    },
    stop_system_capture: function () {
      fireState('processing');
      return stopSystemCapture()
        .then(uploadAudio)
        .then(function (r) {
          if (!r || !r.success) throw new Error((r && r.error) || 'Transcription failed');
          fireState('success');
          try { window.dispatchEvent(new Event('wc-history-refresh')); } catch (e) {}
          return { success: true, text: r.text, id: r.id };
        })
        .catch(function (e) { fireState('error', e.message); return { success: false, error: e.message }; });
    },
    // WC-PREMIUM:END
