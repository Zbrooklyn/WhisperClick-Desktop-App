...
class Sidecar extends EventEmitter {
  ...
  // Commands that may take a long time (model downloads, whole-file transcription).
  static LONG_TIMEOUT_COMMANDS = new Set(['download_model', 'transcribe_file']);
  // LLM enrichment (summary/chat/action) can run long on big transcripts — give
  // it more than the 60s default but well under a model-download budget.
  static MEDIUM_TIMEOUT_COMMANDS = new Set([
    // WC-PREMIUM:START
    'summarize', 'chat_note', 'run_action'
    // WC-PREMIUM:END
  ]);
  ...
  async send(command, payload = {}) {
    ...
    const timeout = Sidecar.LONG_TIMEOUT_COMMANDS.has(command) ? 600000
      : Sidecar.MEDIUM_TIMEOUT_COMMANDS.has(command) ? 120000 : 60000;
