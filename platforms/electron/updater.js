...
function initUpdater(mainWindow, store, sidecar) {
  ...
  // Guard the autoUpdater setup: in a dev/unpackaged build electron-updater
  // throws (no app-update.yml). Without this guard the throw skipped the IPC
  // handler registration below, so the renderer got "No handler registered for
  // 'get-update-channel'". The handlers must always register.
  try {
  ...
  autoUpdater.on('error', (err) => {
    ...
    // A build handed out before its update feed is live gets a 404 or a DNS
    // failure on every launch. That is the feed not existing yet, not the app
    // breaking, so it must not read as a red failure — say plainly that there
    // is nowhere to check and leave the real error for genuine faults.
    // ENOENT/app-update.yml is the same situation seen from the other side: the
    // build has no feed configured at all. Either way the user must never be
    // shown a raw file path or HTTP status in the settings panel.
    if (/\b404\b|ENOTFOUND|EAI_AGAIN|ERR_NAME_NOT_RESOLVED|ECONNREFUSED|getaddrinfo|app-update\.yml|ENOENT/i.test(msg)) {
      sendStatus({ status: 'unavailable', message: 'No update server for this build yet.' });
      return;
    }
    ...
    try { console.warn('[updater] setup skipped (dev/unpackaged?):', err && err.message); } catch { /* ignore */ }
  }

  // --- IPC handlers (registered unconditionally, outside the guarded setup) ---
  ...
  ipcMain.handle('set-update-channel', (_, channel) => {
    ...
    // Applying to autoUpdater can throw in dev — the persisted setting is the
    // source of truth, so swallow and let the round-trip succeed.
    try {
      autoUpdater.channel = valid === 'beta' ? 'beta' : 'latest';
      autoUpdater.allowPrerelease = valid === 'beta';
    } catch { /* dev/unpackaged */ }
