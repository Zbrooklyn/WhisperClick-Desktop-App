...
// --- Premium gating ---
// Pro features are active only when the premium module is present (paid build)
// and the user hasn't disabled them. Real license enforcement layers on later;
// for now this is an honest on/off, never a dead button.
function premiumOn() {
...
ipcMain.handle('pro-activate', async (_, key) => {
  if (!premiumOn() || !proLicense || !proLicense.activate) {
...
ipcMain.handle('pro-deactivate', async () => {
  if (!premiumOn() || !proLicense || !proLicense.deactivate) {
    return { ok: false, error: 'licensing-unavailable' };
  }
  try {
...
// WC-PREMIUM:START
// Bridge meeting start/stop from the injected meeting UI to the engine.
ipcMain.handle('meeting-command', (_, command) => {
  if (!premiumOn() || !sidecar || !sidecar.isRunning) {
    return { ok: false, error: 'engine-unavailable' };
  }
  if (command !== 'start_meeting' && command !== 'stop_meeting') {
    return { ok: false, error: 'unknown-command' };
  }
  sidecar.send(command).catch(() => {});
  return { ok: true };
});
// WC-PREMIUM:END
...
// WC-PREMIUM:START
// ── Premium AI / enrichment parity ──────────────────────────────────────────
// The shared frontend calls summarize/chat_note/run_action/diarize/send_integration
// directly via callNativeApi. Mirror the web build (server.js): the first three run
// through the same Python engine (already keyed by configureSidecar); diarize and
// send_integration are direct HTTP, replicated here.
function aiGate() {
  if (!premiumOn()) return { success: false, error: 'Premium features are off.' };
  if (!sidecar || !sidecar.isRunning) return { success: false, error: 'Backend not ready — restarting…' };
  const s = store.getSettings();
  if (!(s.openaiApiKey && s.openaiApiKey.trim()) && !(s.geminiApiKey && s.geminiApiKey.trim())) {
    return { success: false, needs_key: true, error: 'Add an OpenAI or Gemini API key in Settings.' };
  }
  return null;
}

ipcMain.handle('summarize', async (_, payload) => {
  const gate = aiGate(); if (gate) return { ...gate, summary: '', action_items: [] };
  const text = (payload && payload.text) || '';
  if (!text.trim()) return { success: true, summary: '', action_items: [] };
  try {
    const r = await sidecar.send('summarize', { text });
    return { success: true, summary: (r && r.summary) || '', action_items: (r && r.action_items) || [] };
  } catch (e) { return { success: false, error: e.message, summary: '', action_items: [] }; }
});

ipcMain.handle('chat_note', async (_, payload) => {
  const gate = aiGate(); if (gate) return gate;
  const { transcript, question, history } = payload || {};
  if (!question || !question.trim()) return { success: false, error: 'Ask a question.' };
  if (!transcript || !transcript.trim()) return { success: false, error: 'No transcript to chat about.' };
  try {
    const r = await sidecar.send('chat_note', { transcript, question, history: history || '' });
    if (r && r.error && !r.reply) return { success: false, error: r.error };
    return { success: true, reply: (r && r.reply) || '' };
  } catch (e) { return { success: false, error: e.message }; }
});

ipcMain.handle('run_action', async (_, payload) => {
  const gate = aiGate(); if (gate) return gate;
  const { transcript, action } = payload || {};
  if (!action || !action.trim()) return { success: false, error: 'No action selected.' };
  if (!transcript || !transcript.trim()) return { success: false, error: 'No transcript to work on.' };
  try {
    const r = await sidecar.send('run_action', { transcript, action });
    if (r && r.error && !r.result) return { success: false, error: r.error };
    return { success: true, result: (r && r.result) || '' };
  } catch (e) { return { success: false, error: e.message }; }
});

// Shared URL/media helpers (same module the web build uses), bound to this
// platform's ffmpeg + engine-venv python.
const os = require('os');
...
const _VENV_PY = process.platform === 'win32'
  ? path.join(__dirname, '../../shared/engine/.venv/Scripts/python.exe')
  : path.join(__dirname, '../../shared/engine/.venv/bin/python');
const _PY_CMD = fs.existsSync(_VENV_PY) ? _VENV_PY : 'python';
const urlImport = createUrlImport({ ffmpeg: _FFMPEG, pyCmd: _PY_CMD });

// url_preview — rich preview of a pasted link (no download). Pure fetch/parse.
ipcMain.handle('url_preview', async (_, payload) => {
  const rawUrl = payload && payload.url;
  if (!rawUrl || !/^https?:\/\//i.test(rawUrl)) return { success: false, error: 'Enter a valid http(s) URL.' };
  try {
    const info = await urlImport.urlPreview(rawUrl);
    return { success: true, ...info };
  } catch (e) { return { success: false, error: 'Could not preview that link.' }; }
});

// import_url — fetch a direct media file or pull audio via yt-dlp, convert to
// WAV, then transcribe through the engine and store it (mirrors server.js).
ipcMain.handle('import_url', async (_, payload) => {
  if (!premiumOn()) return { success: false, error: 'Premium features are off.' };
  if (!sidecar || !sidecar.isRunning) return { success: false, error: 'Backend not ready — restarting…' };
  const rawUrl = payload && payload.url;
  if (!rawUrl || !/^https?:\/\//i.test(rawUrl)) return { success: false, error: 'Enter a valid http(s) URL.' };
  ...
  const wavPath = path.join(tmpDir, `url-${stamp}.wav`);
  let extM = null;
  try { extM = new URL(rawUrl).pathname.match(urlImport.MEDIA_EXT_RE); } catch {}
  const useYtdlp = !extM && urlImport.ytdlpAvailable();
  let tmpMedia = null;
  try {
    if (useYtdlp) {
      tmpMedia = urlImport.ytdlpExtractAudio(rawUrl, path.join(tmpDir, `url-${stamp}`));
      urlImport.toWav(tmpMedia, wavPath);
    } else {
      const ext = extM ? '.' + extM[1].toLowerCase() : '.bin';
      // Distinct basename from wavPath — a direct .wav source would otherwise
      // collide with the output path and ffmpeg exits ("same as Input").
      tmpMedia = path.join(tmpDir, `url-${stamp}-src${ext}`);
      const resp = await fetch(rawUrl, { redirect: 'follow' });
      if (!resp.ok) return { success: false, error: 'Could not fetch that URL (HTTP ' + resp.status + ').' };
      const ct = (resp.headers.get('content-type') || '').toLowerCase();
      if (ext === '.bin' && !/audio|video|octet-stream|mpeg|mp4|ogg|webm|wav|mpga/.test(ct)) {
        return { success: false, error: 'That link is not a media file. Paste a YouTube link or a direct audio/video URL.' };
      }
      const buf = Buffer.from(await resp.arrayBuffer());
      if (!buf.length) return { success: false, error: 'The URL returned no data.' };
      fs.writeFileSync(tmpMedia, buf);
      urlImport.toWav(tmpMedia, wavPath);
    }
    setAppState('processing');
    broadcastState();
    // The engine transcribes wavPath; its `transcription` event persists the entry.
    const r = await transcribeFileViaEngine(wavPath);
    if (!r.success) return r;
    return { success: true, id: r.id, text: (r.text || '').trim() };
  } catch (e) {
    return { success: false, error: (e && e.message) ? String(e.message).slice(0, 240) : 'Import failed.' };
  } finally {
    if (tmpMedia) { try { fs.unlinkSync(tmpMedia); } catch {} }
  }
});

// System-audio capture: the renderer (preload) records loopback audio via
// getDisplayMedia + MediaRecorder and hands us the bytes; we write a temp file,
// convert to WAV, and transcribe through the engine (mirrors the web build, which
// POSTs the blob to /api/transcribe).
ipcMain.handle('system_capture_save', async (_, payload) => {
  if (!premiumOn()) return { success: false, error: 'Premium features are off.' };
  if (!sidecar || !sidecar.isRunning) return { success: false, error: 'Backend not ready — restarting…' };
  const bytes = payload && payload.bytes;
  const mime = (payload && payload.mime) || 'audio/webm';
  if (!bytes || !bytes.length) return { success: false, error: 'No audio captured.' };
  const stamp = Date.now().toString(36) + Math.random().toString(36).slice(2, 8);
  const ext = /mp4|m4a/.test(mime) ? '.mp4' : /ogg/.test(mime) ? '.ogg' : /wav/.test(mime) ? '.wav' : '.webm';
  const tmpDir = os.tmpdir();
  const rawPath = path.join(tmpDir, `sys-${stamp}-src${ext}`);
  const wavPath = path.join(tmpDir, `sys-${stamp}.wav`);
  try {
    fs.writeFileSync(rawPath, Buffer.from(bytes));
    urlImport.toWav(rawPath, wavPath);
    setAppState('processing');
    broadcastState();
    const r = await transcribeFileViaEngine(wavPath);
    return r.success ? { success: true, id: r.id, text: (r.text || '').trim() } : r;
  } catch (e) {
    return { success: false, error: (e && e.message) ? String(e.message).slice(0, 240) : 'System capture failed.' };
  } finally {
    try { fs.unlinkSync(rawPath); } catch {}
  }
});

// Transcribe a file on disk through the engine. Mirrors the web build's
// engine.transcribeFile: the text arrives on the `transcription` EVENT, not the
// command reply. main's own transcription listener (registered at startup, so it
// runs first) already persists the history entry — we just read the newest id
// back, never adding a second time.
function transcribeFileViaEngine(filePath, { claim = false, timeoutMs = 180000 } = {}) {
  return new Promise((resolve) => {
    let done = false;
    // claim=true → this caller persists the result itself; the global listener
    // must not also add a new entry. Released in cleanup on every exit path.
    const token = claim ? {} : null;
    if (token) _transcriptionClaim = token;
    const cleanup = () => {
      clearTimeout(timer);
      if (token && _transcriptionClaim === token) _transcriptionClaim = null;
      sidecar.removeListener('transcription', onDone);
      sidecar.removeListener('error', onErr);
    };
    const onDone = (data) => {
      if (done) return; done = true;
      const newest = claim ? null : store.getHistory()[0];
      cleanup();
      resolve({ success: true, text: (data && data.text) || '', data: data || {}, id: newest && newest.id });
    };
    const onErr = (data) => {
    ...
    sidecar.send('transcribe_file', { path: filePath }).catch((e) => {
      if (done) return; done = true; cleanup();
...
// File upload — the renderer resolves the real path via webUtils (preload) and
// the engine transcribes it, landing in history like any other take.
ipcMain.handle('upload_file', async (_, payload) => {
  const filePath = payload && payload.path;
  if (!filePath || !fs.existsSync(filePath)) return { success: false, error: 'File not found.' };
  if (!sidecar || !sidecar.isRunning) return { success: false, error: 'Backend not ready — restarting…' };
  setAppState('processing');
  broadcastState();
  return await transcribeFileViaEngine(filePath);
});
// WC-PREMIUM:END

// WC-PREMIUM:START
// Re-transcribe a stored recording with a higher-accuracy API model, then update
// the entry IN PLACE (mirrors server.js). Temporarily reconfigures the engine to
// the chosen provider and always restores the user's real config afterward.
ipcMain.handle('retranscribe', async (_, payload) => {
  if (!premiumOn()) return { success: false, error: 'Premium features are off.' };
  if (!sidecar || !sidecar.isRunning) return { success: false, error: 'Backend not ready — restarting…' };
  const { id, provider } = payload || {};
  const item = store.getHistory().find((h) => h.id === id);
  const wav = item && item.audio_file;
  if (!item || !wav || !fs.existsSync(wav)) return { success: false, error: 'No stored audio for this transcript.' };

  const s = store.getSettings();
  const prov = (provider === 'openai' || provider === 'gemini')
    ? provider
    : (s.openaiApiKey ? 'openai' : (s.geminiApiKey ? 'gemini' : null));
  const key = prov === 'gemini' ? s.geminiApiKey : (prov === 'openai' ? s.openaiApiKey : '');
  if (!prov || !key) {
    return { success: false, needs_key: true, error: 'Add an OpenAI or Gemini API key in Settings to re-transcribe with a better model.' };
  }
  const apiModel = prov === 'gemini' ? 'gemini-1.5-flash' : 'gpt-4o-transcribe';
  try {
    await sidecar.send('configure', {
      mode: 'api', model: s.localModel, language: s.language, provider: prov,
      api_key: key, api_model: apiModel, base_url: s.customBaseUrl || '',
      output_mode: 'transcribe', target_language: s.targetLanguage, source_language: s.sourceLanguage,
      sound_enabled: false,
    });
    // claim: we persist this result ourselves (update, not add).
    const r = await transcribeFileViaEngine(wav, { claim: true });
    if (!r.success) return r;
    const text = (r.text || '').trim();
    if (!text) return { success: false, error: 'Re-transcription returned nothing.' };
    const d = r.data || {};
    store.updateHistory(id, {
      text, provider: 'api', model: apiModel,
      language: d.language || item.language,
      words: (Array.isArray(d.words) && d.words.length) ? d.words : item.words,
      retranscribed: true,
    });
    return { success: true, text, provider: prov, model: apiModel };
  } catch (e) {
    return { success: false, error: e.message };
  } finally {
    // Always restore the user's real engine config.
    configureSidecar();
  }
});

// Speaker diarization — direct OpenAI call on the stored WAV (mirrors server.js).
ipcMain.handle('diarize', async (_, payload) => {
  if (!premiumOn()) return { success: false, error: 'Premium features are off.' };
  ...
  if (!item || !wav || !fs.existsSync(wav)) return { success: false, error: 'No stored audio for this transcript.' };
  const s = store.getSettings();
  if (!(s.openaiApiKey && s.openaiApiKey.trim())) return { success: false, needs_key: true, error: 'Add an OpenAI API key in Settings to identify speakers.' };
  try {
    ...
    const base = (s.customBaseUrl && /^https?:\/\//.test(s.customBaseUrl)) ? s.customBaseUrl.replace(/\/$/, '') : 'https://api.openai.com/v1';
    const resp = await fetch(base + '/audio/transcriptions', { method: 'POST', headers: { Authorization: 'Bearer ' + s.openaiApiKey }, body: form });
    const j = await resp.json().catch(() => ({}));
    if (!resp.ok) return { success: false, error: (j.error && j.error.message) || ('OpenAI error ' + resp.status) };
    const segments = (j.segments || []).map((seg) => ({ speaker: seg.speaker, text: seg.text, start: seg.start, end: seg.end }));
    return { success: true, segments, text: j.text || '', suggestions: [], people: [] };
  } catch (e) { return { success: false, error: e.message }; }
});

// Send-to integrations — Slack incoming webhook / Notion page (tokens in settings).
ipcMain.handle('send_integration', async (_, payload) => {
  const { text, target } = payload || {};
  if (!text || !text.trim()) return { success: false, error: 'Nothing to send.' };
  const s = store.getSettings();
  try {
    if (target === 'slack') {
      const url = s.slackWebhookUrl || s.slack_webhook_url;
      if (!url) return { success: false, error: 'Add a Slack Incoming Webhook URL in Settings → Premium.' };
      const r = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text }) });
      return { success: r.ok, error: r.ok ? undefined : ('Slack returned ' + r.status) };
    }
    if (target === 'notion') {
      const token = s.notionToken || s.notion_token;
      const db = s.notionDatabaseId || s.notion_database_id;
      if (!token || !db) return { success: false, error: 'Add your Notion token + database ID in Settings → Premium.' };
      ...
      return { success: r.ok, error: r.ok ? undefined : (j.message || ('Notion returned ' + r.status)) };
    }
...
app.whenReady().then(() => {
  ...
  // WC-PREMIUM:START
  // Premium (meeting mode): relay engine meeting events into the injected UI.
  // Each handler updates the meeting panel in the renderer; no-op when the
  // module or window is absent.
  sidecar.on('meeting_status', (data) => {
    if (!premiumOn() || !proMeetingUi || !proMeetingUi.onMeetingStatus) return;
    if (!mainWindow || mainWindow.isDestroyed()) return;
    try { proMeetingUi.onMeetingStatus(mainWindow, data); } catch (e) { log.error('meeting_status relay failed: ' + e); }
  });
  sidecar.on('meeting_partial', (data) => {
    if (!premiumOn() || !proMeetingUi || !proMeetingUi.onMeetingPartial) return;
    if (!mainWindow || mainWindow.isDestroyed()) return;
    try { proMeetingUi.onMeetingPartial(mainWindow, data); } catch (e) { log.error('meeting_partial relay failed: ' + e); }
  });
  sidecar.on('meeting_result', (data) => {
    if (!premiumOn() || !proMeetingUi || !proMeetingUi.onMeetingResult) return;
    if (!mainWindow || mainWindow.isDestroyed()) return;
    try { proMeetingUi.onMeetingResult(mainWindow, data); } catch (e) { log.error('meeting_result relay failed: ' + e); }
  });
  // WC-PREMIUM:END
