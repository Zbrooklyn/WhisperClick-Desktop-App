...
contextBridge.exposeInMainWorld('pywebview', {
  api: {
    ...
    // WC-PREMIUM:START
    async start_meeting() {
      return await ipcRenderer.invoke('meeting-command', 'start_meeting');
    },
    async stop_meeting() {
      return await ipcRenderer.invoke('meeting-command', 'stop_meeting');
    },

    // ── Media in ───────────────────────────────────────────────────────────
    // The shared UI hands us a File. Electron 40 removed File.path, so resolve
    // the real filesystem path via webUtils and let the engine transcribe it.
    async upload_file(file) {
      let filePath = '';
      try { filePath = webUtils.getPathForFile(file); } catch { filePath = (file && file.path) || ''; }
      if (!filePath) return { success: false, error: 'Could not read that file.' };
      return await ipcRenderer.invoke('upload_file', { path: filePath });
    },

    async retranscribe(id, provider) {
      return await ipcRenderer.invoke('retranscribe', { id, provider: provider || '' });
    },
    async import_url(url) {
      return await ipcRenderer.invoke('import_url', { url });
    },
    async url_preview(url) {
      return await ipcRenderer.invoke('url_preview', { url });
    },

    // System-audio capture: record the machine's output via getDisplayMedia
    // loopback (main grants it, no picker), then hand the bytes to main for
    // conversion + transcription (mirrors web-shim's getDisplayMedia path).
    async start_system_capture() {
      try {
        const stream = await navigator.mediaDevices.getDisplayMedia({ video: true, audio: true });
        const audio = stream.getAudioTracks();
        if (!audio.length) { stream.getTracks().forEach((t) => t.stop()); return { success: false, error: 'No system audio was captured. Make sure something is playing.' }; }
        stream.getVideoTracks().forEach((t) => t.stop()); // audio only
        _sysStream = stream; _sysChunks = []; _sysMime = _pickSysMime();
        const audioOnly = new MediaStream(audio);
        _sysRecorder = _sysMime ? new MediaRecorder(audioOnly, { mimeType: _sysMime }) : new MediaRecorder(audioOnly);
        _sysRecorder.ondataavailable = (e) => { if (e.data && e.data.size) _sysChunks.push(e.data); };
        _sysRecorder.start();
        try { window.dispatchEvent(new CustomEvent('state-update', { detail: { state: 'recording' } })); } catch (e) {}
        return { success: true };
      } catch (e) { return { success: false, error: e.message || 'System audio capture failed.' }; }
    },
    async stop_system_capture() {
      return new Promise((resolve) => {
        if (!_sysRecorder) return resolve({ success: false, error: 'not capturing' });
        _sysRecorder.onstop = async () => {
          const blob = new Blob(_sysChunks, { type: _sysMime || 'audio/webm' });
          if (_sysStream) { try { _sysStream.getTracks().forEach((t) => t.stop()); } catch (e) {} _sysStream = null; }
          _sysRecorder = null;
          try {
            const buf = new Uint8Array(await blob.arrayBuffer());
            resolve(await ipcRenderer.invoke('system_capture_save', { bytes: buf, mime: blob.type }));
          } catch (e) { resolve({ success: false, error: e.message }); }
        };
        try { _sysRecorder.stop(); } catch (e) { resolve({ success: false, error: e.message }); }
      });
    },

    // ── Premium AI / enrichment (parity with the web build) ─────────────────
    async summarize(text) {
      return await ipcRenderer.invoke('summarize', { text });
    },
    async chat_note(transcript, question, history) {
      return await ipcRenderer.invoke('chat_note', { transcript, question, history: history || '' });
    },
    async run_action(transcript, action) {
      return await ipcRenderer.invoke('run_action', { transcript, action });
    },
    async diarize(id) {
      return await ipcRenderer.invoke('diarize', { id });
    },
    async send_integration(text, target) {
      return await ipcRenderer.invoke('send_integration', { text, target });
    },
    // WC-PREMIUM:END
