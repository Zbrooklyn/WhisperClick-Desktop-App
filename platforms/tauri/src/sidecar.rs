                    ...
                    Ok(resp) => {
                        if let Some(event) = &resp.event {
                            ...
                            if let Some(cb) = pending.remove(&id) {
                                // Match rather than is_some()/unwrap(): the two
                                // are equivalent today only because they sit
                                // next to each other, and the unwrap is a panic
                                // waiting for the day they don't.
                                match resp.error {
                                    Some(err) => cb(Err(err)),
                                    None => cb(Ok(resp)),
                                }
...
mod tests {
    ...
    fn parse_response_with_extra_unknown_fields() {
        ...
        assert!(resp.extra.contains_key("nested"));
    }
