...
fn stop_recording(
    ...
    // Send stop_rec to sidecar — return immediately.
    // The sidecar "transcription" event handler (line ~1238) handles:
    //   - Transition to Success + broadcast
    //   - History, clipboard, auto-paste
    // The "cancelled" event handler (line ~1363) handles cancel.
    // The frontend learns about completion via real-time state-update events.
...
fn send_configure(sc: &Sidecar, store: &Store) {
    ...
    params.insert("api_key".into(), Value::String(api_key));
    params.insert(
        "base_url".into(),
        Value::String(
            s.get("customBaseUrl")
        ...
        .setup(|app| {
            ...
            // --- Main window (responsive sizing + bridge.js injection) ---
            {
                ...
                let (win_w, win_h) = if let Some(monitor) = app.primary_monitor().ok().flatten() {
                    ...
                    let w = (screen_w * 0.22).clamp(480.0, 650.0);
